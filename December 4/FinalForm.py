...
class Option():
  ...
  finished = ''
  score = 0
  def __init__(self, date, time, finished, score, display):
    self.date = date
    self.time = time
  ...
  def post(self):
    ...
    for p in q:
      if (eventOccursOnDate(p, target) == "yes" or allEvents == 1): #if the event and the target date overlap (taking repeats into consideration)
        formattedStart = formatTimeBetter(p.start)
        formattedEnd = formatTimeBetter(p.end)
        text += ('<tr>')
        text += ('<td><b>' + p.name + '</b></td></tr><tr><td></td>')
        text += ('<td>' + p.date + '</td>')
        text += ('<td>' + formattedStart + '</td>')
        text += ('<td>' + formattedEnd + '</td>')
...
class AjaxSendFrinvitation(webapp2.RequestHandler):
  def post(self):
    ...
    if (not recipient):
      self.response.out.write("Must enter some name")
    else:
    ...
    if (stupidDate == "stupid" or stupidDate2 == "stupid"):
      ...
      if (not eName or not earliestStart or not latestStart or not first or not last):  #if something was left null
        ...
        if (end > 1440):    #if the event extends into the next day
          ...
          thing = 'Span of days must not exceed 10'
          template_values = {'message': thing, 'indexNumber1': 1}
          self.response.out.write(template.render('HTML/newIndex.html', template_values))
          ...
          for w in validStarts:      #for each possible start time
            ...
            showTime = formatTimeBetter(w.time)
            options.append(Option(w.date, w.time, finished, percentScore, showTime)) #add an option for start w with its calculated overall score
...
class AjaxEnvitations(webapp2.RequestHandler):    #displays all Envitations
  ...
  #show its information
      ...
      text += '<p>Start: ' + formattedStart + '</p>'
      text += '<p>End: ' + formattedEnd + '</p>'
      key2 = str(p.key())
...
def formatTimeBetter(time):
  pieces = time.split(':')
  hours = int(pieces[0])
  minutes = int(pieces[1])
  if (hours >= 12):      #after noon
    if (hours == 13):    #13 goes to 1
      hours = 1
    elif (hours == 14):
      hours = 2
    elif (hours == 15):
      ...
      hours = 11
    if (minutes < 10):  #add a leading zero for minutes, hours are fine with no leading zero
      return (str(hours) + ":0" + str(minutes) + "PM")
    else:
      return (str(hours) + ":" + str(minutes) + "PM")
  else:
    if (hours == 0):    #0 corresponds to 12AM
      hours = 12
    if (minutes < 10):
      return (str(hours) + ":0" + str(minutes) + "AM")
    else:
